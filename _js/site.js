$(document).ready(function(e) {
    ...
    /*navigasyon*/
    $(document).on("click", "#nav_article a", function() {
        $.ajax({
            data: {
                frm: 'article',
    ...
    /*Üyelik*/
    $("#sifre1,#sifre2").blur(function() {
        if ($("#sifre1").val() != $("#sifre2").val()) $("#hata").show(100);
        else $("#hata").hide();
    });
    ...
    $(document).on("submit", "#frmGiris", function(event) {
        $.ajax({
            ...
            success: function(gelen) {
                ...
                $("#giris").html(data[0]);
                mesaj(data);
                if (!data[2]) location.href = "anasayfa";
            }
        });
        return false;
    });
    ...
    $(document).on("click", "#mesaj .alert button.close,#alert", function() {
        if ($("#mesaj .alert").hasClass('alert-success') || $("#mesaj .alert").hasClass('alert-info')) {
            location.href = 'anasayfa';
        } else {
            $("#mesaj").fadeOut('slow');
        }
    ...
    function mesaj(data) {
        if (data[1]) {
            $("#mesaj").show();
            $("#mesaj .alert").removeClass('alert-info').removeClass('alert-success').removeClass('alert-warning').removeClass('alert-danger').addClass('alert-' + data[1]);
            $("#mesaj p").html(data[2]);
		}
